...
import { brd, idObjs, idFuns, reqid} from "./Almacen";
...
const BorraRectaTang= () => {
  let objs;
  let board;
  idObjs.subscribe(valor => {objs= valor});
  brd.subscribe(b => {board=b});
  while (objs.length > 0) {
...
const BorraGrafDer = () => {
  let funs;
  let board;
  idFuns.subscribe(valor => {funs= valor});
  brd.subscribe(b => {board=b});
  if (funs.length > 1) {
...
const xInicialGlider= (brd, func) => {
  const ptosInter= DesdePixeles(
    [new Punto(0,0), new Punto(30,0)], brd
  );
...
const AnimaRT= (brd, param) =>{
  ...
  function animaGlider(timestamp) {
    ...
    if (pg.X() >= xfin) {
      ...
      reqid.update(() => requestAnimationFrame(animaGlider));
    }
  }
  reqid.update(() => requestAnimationFrame(animaGlider));
}
...
const MuestraRT = (brd, param) => {
  ...
  const pga = brd.create("point", [() => pg.X() - seg, () => rt(pg.X() - seg)], {
    name: "",
    size: 1,
    visible: true,
  });
  const pgb = brd.create("point", [() => pg.X() + seg, () => rt(pg.X() + seg)], {
    name: "",
    size: 1,
    visible: true,
  });
  ...
  const pend= brd.create("text", [() => pg.X() + seg/2, () => pg.Y(),
              () => `P'(${pg.X().toFixed(2)}) = ${df(pg.X()).toFixed(2)}`] , {anchorY: "middle"} );
